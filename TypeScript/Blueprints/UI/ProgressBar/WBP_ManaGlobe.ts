...
export interface WBP_ManaGlobe extends UE.Game.Blueprints.UI.ProgressBar.WBP_ManaGlobe.WBP_ManaGlobe_C {
}

// 创建一个继承ts的本体类    implements   实现类型提示
@mixin(AssetPath)
export class WBP_ManaGlobe extends WBP_GlobeProgressBar implements WBP_ManaGlobe {
    ...
    OnCurrentChanged(NewValue: number) {
        this.Mana = NewValue
        const pre = (this.MaxMana != 0) ? (this.Mana / this.MaxMana) : 0
        this.SetProgressBarPercent(pre)
    }

    // 监听最大属性变化
    OnMaxChanged(NewValue: number) {
        this.MaxMana = NewValue
        const pre = (this.MaxMana != 0) ? (this.Mana / this.MaxMana) : 0
        this.SetProgressBarPercent(pre)
    }
}